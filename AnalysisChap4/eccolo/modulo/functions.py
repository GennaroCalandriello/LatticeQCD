...
def IPR(data, kind):
    ...
    IPR_mean = np.array(IPR_mean) ** (-1)
    print("lunghezza ipr", len(IPR_mean))
    plt.figure()
    plt.title(
        r"Participation Ratio $\cdot$ four-volume vs $\lambda$" f" for {kind} phase",
        fontsize=13,
    )
    plt.xlabel(r"$\langle \lambda \rangle$", fontsize=15)
    plt.ylabel(r"$V\cdot \langle PR \rangle$", fontsize=15)
