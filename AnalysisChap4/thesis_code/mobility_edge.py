...
def spectralRegionsAnalysis(spectral_window):
    ...
    if lambda_edge_with_KDE:
        lamb = lambda_edge_via_Is0(Is0_with_kde, lambda_values)

    else:
        lamb = lambda_edge_via_Is0(Is0, lambda_values)
...
def mobility_edge():
    ...
    DeltaLambda =np.linspace(0.0015, 0.008, 20) #here various spectral regions
    lambda_edge = []
    for deltaL in DeltaLambda:
        print("spectral window: ", deltaL)
        lambda_c =spectralRegionsAnalysis(deltaL)
        lambda_edge.append([lambda_c, deltaL])
        print(lambda_edge)
...
def lambda_estimate():
    ...
    standard_error = std/np.sqrt(len(Lamb))
    print("mean lambda_c", mean)
    print("std lambda_c", std)
